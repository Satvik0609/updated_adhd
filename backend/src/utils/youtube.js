      ...
      (resp) => {
        ...
        resp.on("end", () => {
          try {
            ...
            if (status >= 400) {
              resolve({ error: true, status, detail: body?.message || "Supadata error" });
              return;
            }
            if (body && typeof body.transcript === "string" && body.transcript.trim().length > 0) {
              resolve({ text: body.transcript });
            } else {
...
export async function tryAssemblyAISummary(url) {
  ...
  function postTranscript() {
    const body = JSON.stringify({ audio_url: url, summarization: true, summary_model: "informative" });
    return new Promise((resolve, reject) => {
      const req = https.default.request(
        "https://api.assemblyai.com/v2/transcript",
        { method: "POST", headers: { authorization: apiKey, "content-type": "application/json", "content-length": Buffer.byteLength(body) } },
        (resp) => {
          const chunks = [];
          resp.on("data", (d) => chunks.push(d));
          resp.on("end", () => {
  ...
  try {
    ...
    if (!created || !created.id) return { error: true, detail: "AssemblyAI: failed to create job" };
    const deadline = Date.now() + 60_000;
    while (Date.now() < deadline) {
      const status = await getTranscript(created.id);
      if (status.status === "completed") return { text: status.text || "", summary: status.summary || "" };
      if (status.status === "error") return { error: true, detail: status.error || "AssemblyAI job error" };
...
export function extractAudioWithFfmpeg(inputPath, outputPath) {
  return new Promise((resolve, reject) => {
    const args = ["-y", "-i", inputPath, "-vn", "-acodec", "libmp3lame", "-ar", "44100", "-ac", "2", "-b:a", "192k", outputPath];
    const proc = execFile("ffmpeg", args, (err) => {
      if (err) reject(new Error(FFmpeg failed: ${err.message})); else resolve(true);
    });
...
export function downloadWithYtDlp(url, outputPath, { extractAudio = false } = {}) {
  return new Promise(async (resolve, reject) => {
    ...
    if (extractAudio) {
      ...
      if (!ffmpegAvailable) return reject(new Error("FFmpeg required for audio extraction but not found"));
    ...
    const proc = execFile("yt-dlp", args, { maxBuffer: 10 * 1024 * 1024 }, (err) => {
      if (err) reject(new Error(err.message)); else resolve(true);
    });
      ...
      const res = await new Promise((resolve, reject) => {
        ...
        https.default.get(apiUrl, (resp) => {
          ...
          if (resp.statusCode && resp.statusCode >= 400) {
            reject(new Error(Piped API status ${resp.statusCode} @ ${host}));
            return;
          }
          resp.on("data", (d) => chunks.push(d));
          resp.on("end", () => {
            const text = Buffer.concat(chunks).toString("utf8");
            if (!/application\/json/i.test(contentType)) {
              reject(new Error(Non-JSON response from ${host}));
              return;
            }
            try { resolve(JSON.parse(text)); } catch (e) { reject(new Error(Invalid JSON from ${host})); }
          });
          resp.on("error", reject);
        }).on("error", reject);
      });
      streams = res && (res.audioStreams || []);
      if (streams && streams.length) break;
    } catch (e) {
      lastErr = e; continue;
    }
  }
  if (!streams || !streams.length) throw new Error(lastErr ? lastErr.message : "No audio streams via Piped");
